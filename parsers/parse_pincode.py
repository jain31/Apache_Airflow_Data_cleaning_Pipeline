...
QUARANTINE_DIR = os.path.join("quarantine", "quarantine_pincode")

# 2. Isolated Logger Setup Configuration for this specific file
def _setup_isolated_logger():
    log_dir = "logger_file"
    os.makedirs(log_dir, exist_ok=True)
    log_filepath = os.path.join(log_dir, "parse_pincode.log")
    
    file_logger = logging.getLogger("parse_pincode")
...
logger.info("Pincode parser execution process initiated.")
...
def parse_pincode(val):
    ...
    if val.lower() == "duplicate":
        return pd.NA
        
    try:
        # Pincodes must be convertible to numbers to be considered valid
        return int(float(val))
    except ValueError:
        # If it contains alphabetical junk strings, route it to quarantine
        return pd.NA
...
def clean_pincode(df, original_df):
    ...
    # Identify items that evaluated to pd.NA (failed processing)
    unparsed_pincode = df['pincode_clean'].isna()
    ...
    if failed_indexes:
        ...
        quarantine_path = os.path.join(
            QUARANTINE_DIR,
            f"quarantine_pincode_{pd.Timestamp.now().strftime('%d-%m-%y')}.csv"
        )
    ...
    clean_df["pincode"] = clean_df["pincode"].astype(int)
